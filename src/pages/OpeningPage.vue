<template>
    <div class="container-fluid">
        <div class="container q-pt-md">
            <div class="row justify-center">
                    <div>
                        <img class="gt-sm q-pt-md" src="https://examsdaily.in/wp-content/uploads/2020/12/examsdaily-new-png-png.png" style="max-width:150px; max-height:150px">
                        <img class="lt-md q-pt-md" src="https://examsdaily.in/wp-content/uploads/2020/12/examsdaily-new-png-png.png" style="max-width:100px; max-height:100px">
                    </div>
                </div>
        </div>
    </div>
    <div class="container-fluid q-pt-xl q-pb-xl">
        <div class="container cdbxx">
            <div class="row justify-around">
                <div class="col-md-7 col-12">
                    <div class="row cd q-pa-md">
                        <q-card flat class="q-pa-sm q-mb-lg bd"  v-for="cds in cd" style="width: 700px;">
                            <div class="row q-pa-md justify-between">
                                <div class="col-md-2 col-sm-6 col-12">
                                    <img class="gt-sm" style="width:150px; height:150px;" :src="cds.imgn">
                                    <img class="lt-md" :src="cds.imgn">
    
                                </div>
                                <div class="cdcnt col-md-8 col-sm-6">
                                    <div class="row justify-between"><div class="col-8 gt-sm text-weight-bolder text-h5">{{ cds.role }}</div>&nbsp;<div class="col-2 gt-sm"><q-btn v-for="size in sizes" :key="`btn_size_sq_${size}`" color="indigo-14" :size="size" :label="`Apply`"/></div></div>
                                    <div class="lt-md cdtle text-weight-bolder text-h5">{{ cds.role }}</div>
                                    <div class="text-grey-13 text-h6"><span>Starbucks</span><br><br></div>
                                    <div class="row justify-evenly q-pt-md text-caption">
                                        <div class="col-md-4 col-12"><span>{{ cds.loctn }}</span> &nbsp;</div>
                                        <div class="col-md-5 col-12"><span><span class="text-grey-13">Date posted:</span> {{ cds.date }}</span> &nbsp;</div>
                                        <div class="col-md-3 col-12 gt-sm"><span><span class="text-grey-13">&nbsp; &nbsp;&nbsp; &nbsp; &nbsp;Openings:</span>{{ cds.openings }}</span> &nbsp;</div>
                                        <div class="col-md-3 col-12 lt-md"><span><span class="text-grey-13">Openings:</span>{{ cds.openings }}</span> &nbsp;</div>
    
                                    </div><br>
                                    <div class="row justify-center">
                                        <div class="col lt-md"><q-btn v-for="size in sizes" :key="`btn_size_sq_${size}`" color="indigo-14" :size="size" :label="`Apply`"/></div>
                                    </div>
                                </div>
                            </div>
                            
                        </q-card>
                        <q-card flat class="q-pa-md q-mb-lg bd"  v-for="cds in cd" style="width: 700px;">
                       <p class="text-h6 text-weight-bolder">Roles and Requirements</p>
                       <p class="text-justify" v-for="n in 1" :key="n">
                           Lorem ipsum dolor sit amet consectetur adipisicing elit. Fugit nihil praesentium molestias a adipisci, dolore vitae odit, quidem consequatur optio voluptates asperiores pariatur eos numquam rerum delectus commodi perferendis voluptate
                       </p>
                            <div>
                                <p>Role: <span class="text-weight-bolder">{{ cds.role }}</span></p>
                                <p>Experience: <span class="text-weight-bolder">{{ cds.exp }}</span></p>
                                <p>Openings: <span class="text-weight-bolder">{{ cds.openings }}</span></p>
                            </div>
                     </q-card>
                     <q-card flat class="q-pa-md bd"  v-for="cds in cd" style="width: 700px;">
                       <p class="text-h6 text-weight-bolder">Who we are ?</p>
                       <p class="text-justify" v-for="n in 1" :key="n">
                           Lorem ipsum dolor sit amet consectetur adipisicing elit. Fugit nihil praesentium molestias a adipisci, dolore vitae odit, quidem consequatur optio voluptates asperiores pariatur eos numquam rerum delectus commodi perferendis voluptate
                       </p>
                            <div>
                                <span>We are also in &nbsp;&nbsp;&nbsp;
                                    <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" fill="currentColor" class="loc bi bi-instagram" viewBox="0 0 16 16">
                                    <path d="M8 0C5.829 0 5.556.01 4.703.048 3.85.088 3.269.222 2.76.42a3.9 3.9 0 0 0-1.417.923A3.9 3.9 0 0 0 .42 2.76C.222 3.268.087 3.85.048 4.7.01 5.555 0 5.827 0 8.001c0 2.172.01 2.444.048 3.297.04.852.174 1.433.372 1.942.205.526.478.972.923 1.417.444.445.89.719 1.416.923.51.198 1.09.333 1.942.372C5.555 15.99 5.827 16 8 16s2.444-.01 3.298-.048c.851-.04 1.434-.174 1.943-.372a3.9 3.9 0 0 0 1.416-.923c.445-.445.718-.891.923-1.417.197-.509.332-1.09.372-1.942C15.99 10.445 16 10.173 16 8s-.01-2.445-.048-3.299c-.04-.851-.175-1.433-.372-1.941a3.9 3.9 0 0 0-.923-1.417A3.9 3.9 0 0 0 13.24.42c-.51-.198-1.092-.333-1.943-.372C10.443.01 10.172 0 7.998 0zm-.717 1.442h.718c2.136 0 2.389.007 3.232.046.78.035 1.204.166 1.486.275.373.145.64.319.92.599s.453.546.598.92c.11.281.24.705.275 1.485.039.843.047 1.096.047 3.231s-.008 2.389-.047 3.232c-.035.78-.166 1.203-.275 1.485a2.5 2.5 0 0 1-.599.919c-.28.28-.546.453-.92.598-.28.11-.704.24-1.485.276-.843.038-1.096.047-3.232.047s-2.39-.009-3.233-.047c-.78-.036-1.203-.166-1.485-.276a2.5 2.5 0 0 1-.92-.598 2.5 2.5 0 0 1-.6-.92c-.109-.281-.24-.705-.275-1.485-.038-.843-.046-1.096-.046-3.233s.008-2.388.046-3.231c.036-.78.166-1.204.276-1.486.145-.373.319-.64.599-.92s.546-.453.92-.598c.282-.11.705-.24 1.485-.276.738-.034 1.024-.044 2.515-.045zm4.988 1.328a.96.96 0 1 0 0 1.92.96.96 0 0 0 0-1.92m-4.27 1.122a4.109 4.109 0 1 0 0 8.217 4.109 4.109 0 0 0 0-8.217m0 1.441a2.667 2.667 0 1 1 0 5.334 2.667 2.667 0 0 1 0-5.334"/>
                                    </svg>&nbsp;
                                    <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" fill="currentColor" class="loc bi bi-facebook" viewBox="0 0 16 16">
                                    <path d="M16 8.049c0-4.446-3.582-8.05-8-8.05C3.58 0-.002 3.603-.002 8.05c0 4.017 2.926 7.347 6.75 7.951v-5.625h-2.03V8.05H6.75V6.275c0-2.017 1.195-3.131 3.022-3.131.876 0 1.791.157 1.791.157v1.98h-1.009c-.993 0-1.303.621-1.303 1.258v1.51h2.218l-.354 2.326H9.25V16c3.824-.604 6.75-3.934 6.75-7.951"/>
                                    </svg>&nbsp;
                                    <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" fill="currentColor" class="loc bi bi-youtube" viewBox="0 0 16 16">
                                    <path d="M8.051 1.999h.089c.822.003 4.987.033 6.11.335a2.01 2.01 0 0 1 1.415 1.42c.101.38.172.883.22 1.402l.01.104.022.26.008.104c.065.914.073 1.77.074 1.957v.075c-.001.194-.01 1.108-.082 2.06l-.008.105-.009.104c-.05.572-.124 1.14-.235 1.558a2.01 2.01 0 0 1-1.415 1.42c-1.16.312-5.569.334-6.18.335h-.142c-.309 0-1.587-.006-2.927-.052l-.17-.006-.087-.004-.171-.007-.171-.007c-1.11-.049-2.167-.128-2.654-.26a2.01 2.01 0 0 1-1.415-1.419c-.111-.417-.185-.986-.235-1.558L.09 9.82l-.008-.104A31 31 0 0 1 0 7.68v-.123c.002-.215.01-.958.064-1.778l.007-.103.003-.052.008-.104.022-.26.01-.104c.048-.519.119-1.023.22-1.402a2.01 2.01 0 0 1 1.415-1.42c.487-.13 1.544-.21 2.654-.26l.17-.007.172-.006.086-.003.171-.007A100 100 0 0 1 7.858 2zM6.4 5.209v4.818l4.157-2.408z"/>
                                    </svg>
                                </span>
                                
                            </div>
                     </q-card>
                    </div>
                    
                </div>
                <div class=" col-md-4 col-sm-12 col-12">
                            <p class="text-weight-bolder gt-sm text-h5 q-pt-md">Similar Jobs</p>
                            <div class="row justify-around">
                                <div class="lt-md cdtpc q-pa-lg text-weight-bolder"><span>Similar Jobs</span></div>
                            </div>
                            <div class="row justify-around">
                                <div class="col-md-12 col-sm-5 q-pb-lg" v-for="jobs in job">
                                    <q-btn flat to="/job" class="bd"  style="width: 370px;">
                                     <div class="row q-pa-xs text-caption justify-between">
                                <div class="col">
                                    <img style="width:55px; height:55px;" :src="jobs.img">
                                </div>
                                <div class="col-11 q-pl-xl cdtx">
                                    <span class="text-weight-bolder btxt">{{ jobs.roles }}</span><br>
                                    <span><q-icon name="work"></q-icon><span>{{jobs.exps}}</span> &nbsp;
                                        <q-icon name="money"></q-icon>{{ jobs.sals }} &nbsp;
                                        <q-icon name="chair"></q-icon>{{ jobs.seats }}&nbsp;
                                        <svg xmlns="http://www.w3.org/2000/svg" width="11" height="11" fill="currentColor" class="bi bi-geo-alt-fill" viewBox="0 0 16 16">
                                        <path d="M8 16s6-5.686 6-10A6 6 0 0 0 2 6c0 4.314 6 10 6 10m0-7a3 3 0 1 1 0-6 3 3 0 0 1 0 6"/>
                                        </svg>{{ jobs.locat }}</span> 
                                </div>
                            </div>
                        </q-btn>
                                </div>
                           
                                <div class="col-md-12 cdtpc col-sm-6">
                                    <q-btn to="/job" class="btxt" style="width: 370px;" color="indigo-14" label="View all Jobs">
                                     </q-btn>
                                </div>
                            </div> 
                        
                        </div>
                         </div>
                </div>
            </div>
            <div class="container-fluid q-pt-xl q-pb-xl">
            </div>
    </template>
    <script>
    const job=[
           {
            img:'data:image/png;base64,',
            ico:'chair',
            roles:'Administrator',
            exps:'2yrs',
            sals:'1lac/yr',
            seats:'10',
            locat:'Madurai'
        },
        {
            img:'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSiy0Bx_2KhS0N20RNtQRvByt0mUSQe1PDz6bakYxXatE9LHsSWq9KUo0605sBkJBhwVyc&usqp=CAU',
            ico:'chair',
            roles:'Administrator',
            exps:'2yrs',
            sals:'1lac/yr',
            seats:'10',
            locat:'Madurai'
        },
        {
            img:'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRxsw9x9h-tP2ha1In0YBipGnfrQqzRf9OS1w&usqp=CAU',
            ico:'chair',
            roles:'Administrator',
            exps:'2yrs',
            sals:'1lac/yr',
            seats:'10',
            locat:'Madurai'
        },
        {
            img:'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSF4fpujQtCRRKmbqVdOxllct5g8EOQeBF_fw&usqp=CAU',
            ico:'chair',
            roles:'Administrator',
            exps:'2yrs',
            sals:'1lac/yr',
            seats:'10',
            locat:'Madurai'
        },
        {
            img:'https://i.pinimg.com/736x/53/32/69/533269d7c75706515c1a44827a535458.jpg',
            ico:'chair',
            roles:'Administrator',
            exps:'2yrs',
            sals:'1lac/yr',
            seats:'10',
            locat:'Madurai'
        },
        {
            img:'https://upload.wikimedia.org/wikipedia/commons/thumb/4/46/Wikipedia-W-visual-balanced.svg/1200px-Wikipedia-W-visual-balanced.svg.png',
            ico:'chair',
            roles:'Administrator',
            exps:'2yrs',
            sals:'1lac/yr',
            seats:'10',
            locat:'Madurai'
            
        },
    
        ]
    
        const cd = [
            {
            imgn:'data:image/png;base64,',
            role:'Administrator',
            loctn:'Kalavasal, Madurai',
            date:'3 Weeks ago',
            openings:'5'
        }
        ]
    
    
    export default{
        
        data(){
            return{
                job,
                cd,
                sizes: [  'sm', ],
            }
        }
    }
    </script>
    <style>
    .cdtx{
    text-align: start !important;
    }
    .btxt{
        text-transform:capitalize;
        font-size: 15px;
    }
    .loc{
        color: grey !important;
    }
    @media(max-width:1000px){
    .cdtpc{
        font-size: 2em;
    }
    }
@media screen and (min-width:425px) and (max-width:600px){
.cdbxx{
    text-align: center !important;
}
}
@media (max-width:425px){
.cdbxx{
    text-align: center !important;
}
}
@media screen and (min-width:600px) and (max-width:1000px){
        .cdtpc{
         margin:auto;
        font-size: 2em;
    }   
    .cdtp{
        text-align:start !important
    }
    }
    @media(max-width:600px){
    .cdcnt{
        text-align: center !important;
    }
    }
    .cdbd{
        border:5px !important;
    }
    .bd{
        border: thin solid rgba(0, 0, 0, 0.046)
    }
    </style>